...
| `max_stroke_mm` | `integer` (mm) | `650` | How far the desk travels above its lowest position. `deskctl goto` refuses a target outside `desk_offset_mm` to `desk_offset_mm + max_stroke_mm` instead of driving into an end stop. The desk reports its base height but not its travel, so this is a setting; the default is the raw height the up button already drives to. Set it to your desk's own travel to tighten the check. Clamped on load to `0`–`6500`, the range a move-to target can be encoded in. |
